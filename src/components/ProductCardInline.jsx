...
export const ProductCardInline = ({ batch, onClick }) => {
  ...
  if (isComingSoon) {
    return (
      <div style={{ background: C.bg, position: 'relative', border: `1px solid ${C.line}`, cursor: 'default' }}>
        <div style={{ aspectRatio: '4/5', background: C.bg2, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden', zIndex: 10 }}>
          {hasImage ? (
            <img
              src={batch.images[0]}
              alt={batch.name}
          ...
          <div style={{ position: 'relative', zIndex: 2, background: 'rgba(13,13,13,0.6)', padding: '6px 12px', border: `1px solid ${C.line}` }}>
  ...
  return (
    <div onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      data-hover
      style={{ background: C.bg, cursor: 'pointer', position: 'relative', border: `1px solid ${hov ? (isClosed ? C.line : C.red) : C.line}`, transition: 'border-color 0.2s' }}>
      ...
      <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
          <span style={{ fontFamily: F.m, fontSize: 13, color: C.ink }}>{batch.price}</span>
